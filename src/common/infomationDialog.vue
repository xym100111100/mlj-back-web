        ...
        <div class="content-table">
          ...
          <table>
            ...
            <tr>
              ...
              <td>
                <span>
                  {{ debtTypeFormat(debtClient.state) }}
                </span>
              </td>
              <th>借款金额：</th>
              <td>
            ...
            <tr>
              ...
              <td>
                <span>
                  {{ backedModeFormat(debtClient.repayType) }}
                </span>
              </td>
            </tr>
          ...
          <div class="btn">
            ...
            <el-button
              v-if="debtClient.state === 1"
              type="warning"
              @click="changeDebtState(3, '是否拒绝该初审')"
              >拒绝初审</el-button
            >
            <el-button
              v-if="debtClient.state === 1"
              type="success"
              @click="changeDebtState(2, '是否同意过初审')"
              >同意初审</el-button
            >
          </div>
          <h3>基础信息</h3>
          <table>
            ...
            <tr>
              ...
              <th>家庭月收入：</th>
              <td>{{ clinetData.familyMonthlyIncome }}</td>
              <th>工作省名称：</th>
            ...
            <el-collapse-item title="查看客户身份证信息" name="1">
              <div>
                <el-row :gutter="20">
                  <el-col
                    :span="11"
                    v-for="(item, i) in clientRealAuthImage"
                    :key="i"
                  >
                    <div class="grid-content">
                      <el-image
                        :preview-src-list="
                          (url = clientRealAuthImage.map(item => item.path))
                        "
                        :src="item.path"
          ...
          <el-collapse v-model="isActive">
            <el-collapse-item title="查看授信附件信息" name="1">
              <div>
                <el-row
                  :gutter="1"
                  v-for="(item, i) in creditDetailImage"
                  :key="i"
                >
                  <el-col
                    :span="7"
                  ...
                  >
                    <div class="grid-content">
                      <el-image
                        :src="item2.path"
                        :preview-src-list="(url = item.map(item => item.path))"
                      ></el-image>
                    </div>
                    <div class="grid-content1">
            ...
            <el-collapse-item title="查看授信附件信息" name="1">
              <div>
                <el-row :gutter="20">
                  <el-col
                    :span="8"
                    v-for="(item, i) in certificateInfo"
                    :key="i"
                  >
                    <div class="grid-content">
                      <el-image
                        :preview-src-list="
                          (url = certificateInfo.map(item => item.path))
                        "
                        :src="item.path"
...
import { getClientLoanDetailsData, modifyDebtStateData } from '@/api/loan'
