          ...
          <el-row :gutter="20">
            <el-col :span="6" v-for="(item, index) in imagesData" :key="index">
              <div class="block">
                ...
                <div>
                  <el-button size="mini" type="danger" @click="delImage(index)"
                    >删除</el-button
                  >
                </div>
              </div>
    ...
    <div class="upload-btn" v-if="dialogStatus.action !== 'info'">
      <div style="margin-right:10px;">
        <el-upload class="upload-demo" action="" :http-request="fileUpload">
          <el-button size="mini" type="primary" @click="willUpload"
            >点击上传</el-button
          >
        </el-upload>
      </div>
  ...
  methods: {
    ...
    willUpload() {
      axios({
        ...
        url:
          'http://tool.jqtianxia.net:9021/upload/signature?fileNumber=1&suffix=jpg&fileType=partner_appendix',
        headers: {
